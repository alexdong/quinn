...
class TestPromptTemplateHandler:
    ...
    def test_load_system_prompt(self) -> None:
        ...
        handler = PromptTemplateHandler()
        system_prompt = handler._load_system_prompt()
        assert isinstance(system_prompt, str)
        assert len(system_prompt) > 0
        assert "Quinn" in system_prompt
    ...
    def test_render_subsequent_prompt(self) -> None:
        ...
        previous_response = "Mostly user profiles and transaction records."

        prompt = handler.render_subsequent_prompt(conversation, previous_response)
        assert isinstance(prompt, str)
        assert len(prompt) > 0
    ...
    def test_render_subsequent_prompt_empty_conversation(self) -> None:
        ...
        with pytest.raises(AssertionError, match="Conversation must have messages"):
            handler.render_subsequent_prompt(conversation, "Some response")

    def test_render_subsequent_prompt_empty_response(self) -> None:
        """Test subsequent prompt rendering fails with empty response."""
        handler = PromptTemplateHandler()
        conversation = Conversation()
        ...
        with pytest.raises(AssertionError, match="Previous response cannot be empty"):
...
class TestConvenienceFunctions:
    ...
    def test_render_subsequent_prompt_function(self) -> None:
        ...
        previous_response = "I want to build web applications."

        prompt = render_subsequent_prompt(conversation, previous_response)
        assert isinstance(prompt, str)
        assert len(prompt) > 0
        assert previous_response in prompt
...
class TestTemplateIntegration:
    ...
    def test_full_conversation_flow(self) -> None:
        ...
        user_response = "We're building a social media platform. Team has mostly frontend experience."
        subsequent_prompt = render_subsequent_prompt(conversation, user_response)

        assert user_response in subsequent_prompt
    ...
    def test_multiple_exchanges(self) -> None:
        ...
        latest_response = "Faster development time and code reuse across platforms."
        prompt = render_subsequent_prompt(conversation, latest_response)
        ...
        assert latest_response in prompt